...
public interface CoursesForGroupRepository extends JpaRepository<CoursesForGroup, Integer> {
    ...
    @Query("select c.course.courseName.name from CoursesForGroup c JOIN StudentDegree s ON s.studentGroup.id = c.studentGroup.id where s.student.email = ?1 and c.course.semester = ?2")
